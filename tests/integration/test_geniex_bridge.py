"""GenieX bridge integration: routing, LLM/VLM request-response, bridge
failure/recovery, timeouts, provider replaceability, and architecture
boundaries."""
import base64
import json
import unittest
...
class GenieXBridgeTests(VerificationTestBase):
    ...
    def test_llm_request_response(self):
        ...
        from core import ModelRequest
        response = port.generate(ModelRequest(
        ...
        self.assertEqual(self.bridge.chat_requests[0]["maxTokens"], 2048)

    def test_vlm_request_response(self):
        def vision(request):
            self.assertEqual(request["model"], "qwen2.5-vl-7b-instruct")
            self.assertEqual(request["prompt"], "describe the screen")
            raw = base64.b64decode(request["imageBase64"])
            self.assertEqual(raw, b"fake-image-bytes")
            return {"content": "a settings screen", "usage": {}}
        self.bridge.vision_handler = vision
    ...
    def test_timeout_handling(self):
        ...
        with self.assertRaises(GenieXBridgeError) as caught:
            client.chat("qwen3-4b-instruct-2507", [{"role": "user",
                                                     "content": "hi"}])
    ...
    def test_http_error_surfaces_stable_code(self):
        ...
        with self.assertRaises(GenieXBridgeError) as caught:
            self.client().chat("qwen3-4b-instruct-2507",
    ...
    def test_provider_replaceability_same_security_outcomes(self):
        ...
        def chat(request):
            if len(self.bridge.chat_requests) == 1:
                return {"content": "", "toolCalls": [good_write.to_dict()],
                        "finishReason": "tool_calls"}
    ...
    def test_provider_replaceability_malicious_calls_equally_denied(self):
        ...
        def chat(request):
            if len(self.bridge.chat_requests) == 1:
                return {"content": "", "toolCalls": [evil.to_dict()],
                        "finishReason": "tool_calls"}
            return {"content": "", "toolCalls": [], "finishReason": "stop"}
    ...
    def test_geniex_tool_calls_still_flow_through_policy(self):
        ...
        def chat(request):
            self.bridge.script_index = getattr(self.bridge, "script_index", 0)
            self.bridge.script_index += 1
            if self.bridge.script_index == 1:
                return {"content": "", "toolCalls": [{
                    "id": "evil", "name": "fs.write_file",
                    "arguments": {"path": "/etc/pwned.txt", "content": "x"}}],
                    "finishReason": "tool_calls"}
            return {"content": "", "toolCalls": [{
                "id": "good", "name": "fs.write_file",
                "arguments": {"path": "/data/out/x.txt", "content": "HELLO"}}],
                "finishReason": "tool_calls"}
